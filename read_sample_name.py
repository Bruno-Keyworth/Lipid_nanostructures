...
def read_sample_name(base_name):
    ...
    lipid_ratio = {"DMPC":0, "DMPG":0, "POPC":0, "POPG":0, "DPPC":0, "DOPC":0}
    ...
    if ratio_match:
        ...
        # -------- single lipid case --------
        single_match = re.search(r"\b(DMPC|DMPG|POPC|POPG|DPPC|DOPC)\b", base_name)
        if single_match:
            lipid_ratio[single_match.group(1)] = 10

    # -------- extrusion number --------
    extr_match = re.search(r"(\d+)\s+(Extrusion|EX)", base_name)
    if extr_match:
